...
public class RefByte extends BaseRefField implements IRefByteField {
    ...
    public static final RefTypeFactory.Factory<RefByte> CREATOR = new RefTypeFactory.Factory<RefByte>() {
        @Override
        public RefByte create(Class<RefByte> fieldType, Class<?> targetClass, String targetName, String targetClassName, Class<?>[] params) {
            return new RefByte(targetClass, targetName, targetClassName, params);
        }
    };

    public RefByte(Class<?> targetClass, String targetName, String targetClassName, Class<?>[] params) {
    ...
    public byte get(Object receiver, byte defValue) {
        try {
            setError(null);
            return target.getByte(receiver);
        } catch (Exception e) {
            setError(e);
            return defValue;
        }
